/**
 * License dispatch + policy integration test (Phase — batch 7, final batch).
 *
 * Proves that the 9 license tools are wired into the dispatcher and that
 ...
 *
 * Mirrors email-e2e.test.ts exactly (proven harness from Batch 6).
 *
 * Tool policy modes from DEFAULT_POLICY (schema.ts):
 *   Reads (allow via list_* and get_* wildcards):
 *     list_license_prices, list_license_items, list_plesk_licenses,
 *     get_plesk_license, get_plesk_key
 *   Allow-writes (explicit allow entries):
 *     update_plesk_license, reset_plesk_hwid
 *   Confirms:
 *     create_plesk_license (billable, confirm-without-spend),
 *     delete_plesk_license (destructive)
 *
...
// The 9 license tools.
import { createListLicensePricesTool } from '../../../src/tools/list-license-prices.js';
import { createListLicenseItemsTool } from '../../../src/tools/list-license-items.js';
import { createListPleskLicensesTool } from '../../../src/tools/list-plesk-licenses.js';
import { createGetPleskLicenseTool } from '../../../src/tools/get-plesk-license.js';
import { createGetPleskKeyTool } from '../../../src/tools/get-plesk-key.js';
import { createCreatePleskLicenseTool } from '../../../src/tools/create-plesk-license.js';
import { createUpdatePleskLicenseTool } from '../../../src/tools/update-plesk-license.js';
import { createResetPleskHwidTool } from '../../../src/tools/reset-plesk-hwid.js';
import { createDeletePleskLicenseTool } from '../../../src/tools/delete-plesk-license.js';
...
describe('License dispatch + policy e2e', () => {
  ...
  beforeAll(async () => {
    ...
    const seeded = await seedTenantOwner(pool, 'license-e2e@example.com', 'x-hash-license');
    const tenantId = seeded.tenant_id;

    operatorKey = await issueTenantKey(pool, tenantId, 'license-operator-key', [
      'mcp:read',
      'mcp:write',
    ]);
    viewerKey = await issueTenantKey(pool, tenantId, 'license-viewer-key', ['mcp:read']);

    const openproviderClient = createOpenproviderClient();

    // dispatchFactory mirrors server.ts wiring exactly.
    async function dispatchFactory(principal: Principal) {
      ...
      try {
        ...
        const tools: DispatcherTool[] = [
          // 5 allow reads (via list_*/get_* wildcards)
    ...
    app = await createMcpServer({
      devToken: 'never-used-license-e2e',
      devPrincipal: {
        kind: 'user',
    ...
    const r = await fetch(`${baseUrl}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'license-e2e', version: '0' },
        },
      }),
    });
  ...
  // ---------------------------------------------------------------------------
  // 1. tools/list / catalog includes all 9 license tool names
  // ---------------------------------------------------------------------------
  it('tools/list includes all 9 license tool names', () => {
  ...
  // ---------------------------------------------------------------------------
  it('operator calling list_license_items (allow-mode read, no args) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'list_license_items', {})) as {
    ...
    expect(
      ...
      'list_license_items should fail with openprovider_not_connected, got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  it('operator calling get_plesk_license (allow-mode read with path id) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'get_plesk_license', { key_id: 1 })) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'get_plesk_license should fail with openprovider_not_connected, got: ' + JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  it('operator calling get_plesk_key (allow-mode read with path id) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'get_plesk_key', { key_id: 1 })) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'get_plesk_key should fail with openprovider_not_connected, got: ' + JSON.stringify(body),
    ).toBe(true);
  }, 60_000);
  ...
  // ---------------------------------------------------------------------------
  it('operator calling reset_plesk_hwid (allow-mode write) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'reset_plesk_hwid', {
      key_id: 1,
      product: 'plesk',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'reset_plesk_hwid should fail with openprovider_not_connected (no confirm), got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  it('operator calling update_plesk_license (allow-mode write) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'update_plesk_license', {
      key_id: 1,
      items: ['SKU'],
      period: 1,
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'update_plesk_license should fail with openprovider_not_connected (no confirm), got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);
  ...
  // ---------------------------------------------------------------------------
  it('operator calling create_plesk_license (confirm-mode billable) → confirmation proposed, not executed', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'create_plesk_license', {
      items: ['SKU'],
      period: 1,
      ip_address_binding: '127.0.0.1',
      title: 'T',
    ...
    expect(
      text,
      'create_plesk_license should return a confirmation proposal (result.content[0].text), got: ' +
        JSON.stringify(body),
    ).toBeDefined();
  ...
  it('operator calling delete_plesk_license (confirm-mode destructive) → confirmation proposed, not executed', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'delete_plesk_license', { key_id: 1 })) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      text,
      'delete_plesk_license should return a confirmation proposal (result.content[0].text), got: ' +
        JSON.stringify(body),
    ).toBeDefined();
  ...
  it('viewer calling delete_plesk_license (confirm-mode write) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'delete_plesk_license', { key_id: 1 })) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error,
      'viewer calling delete_plesk_license should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // 5b. Viewer calling confirm-mode billable write → policy_denied
  it('viewer calling create_plesk_license (confirm-mode billable) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'create_plesk_license', {
      items: ['SKU'],
      period: 1,
      ip_address_binding: '127.0.0.1',
      title: 'T',
    ...
    expect(
      body.error,
      'viewer calling create_plesk_license should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // 5c. Viewer calling allow-mode write → policy_denied
  it('viewer calling reset_plesk_hwid (allow-mode write) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'reset_plesk_hwid', {
      key_id: 1,
      product: 'plesk',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error,
      'viewer calling reset_plesk_hwid should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // 5d. Viewer calling allow-mode write → policy_denied
  it('viewer calling update_plesk_license (allow-mode write) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'update_plesk_license', {
      key_id: 1,
      items: ['SKU'],
      period: 1,
      ip_address_binding: '127.0.0.1',
    ...
    expect(
      body.error,
      'viewer calling update_plesk_license should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // 5e. Viewer calling allow-mode read → allowed, reaches handler → not-connected
  it('viewer calling list_license_items (allow-mode read) → allowed → openprovider_not_connected', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'list_license_items', {})) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'viewer calling list_license_items should reach handler and fail with openprovider_not_connected, got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  // 5f. Viewer calling allow-mode read with args → allowed, reaches handler → not-connected
  it('viewer calling get_plesk_license (allow-mode read with id) → allowed → openprovider_not_connected', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'get_plesk_license', { key_id: 1 })) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'viewer calling get_plesk_license should reach handler and fail with openprovider_not_connected, got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);
});
