/**
 * Catalog + tag dispatch + policy integration test (Phase — batch 3).
 *
 * Proves that the 6 new catalog/tag tools are wired into the dispatcher and
 * that DEFAULT_POLICY modes (allow vs confirm) and the viewer write-gate behave
 * correctly end-to-end through the real HTTP stack, without making real
 * Openprovider network calls.
 *
 * Mirrors the dns-e2e.test.ts Phase-2 harness exactly:
...
import { requiredApproverRoles, DEFAULT_POLICY, type Role } from '../../../src/policies/schema.js';
...
// The 6 catalog/tag tools.
import { createListTldsTool } from '../../../src/tools/list-tlds.js';
import { createGetTldTool } from '../../../src/tools/get-tld.js';
import { createGetDomainPriceTool } from '../../../src/tools/get-domain-price.js';
import { createListTagsTool } from '../../../src/tools/list-tags.js';
import { createCreateTagTool } from '../../../src/tools/create-tag.js';
import { createDeleteTagTool } from '../../../src/tools/delete-tag.js';
...
describe('catalog + tag dispatch + policy e2e', () => {
  ...
  beforeAll(async () => {
    ...
    const seeded = await seedTenantOwner(pool, 'catalog-e2e@example.com', 'x-hash-catalog');
    const tenantId = seeded.tenant_id;

    operatorKey = await issueTenantKey(pool, tenantId, 'catalog-operator-key', [
      'mcp:read',
      'mcp:write',
    ]);
    viewerKey = await issueTenantKey(pool, tenantId, 'catalog-viewer-key', ['mcp:read']);
    ...
    //   - all 6 new catalog/tag tools
    //   - no real OP creds so list_tlds / create_tag will throw not-connected
    async function dispatchFactory(principal: Principal) {
      ...
      try {
        ...
        const tools: DispatcherTool[] = [
          // 4 allow reads (covered by list_* / get_* wildcards in DEFAULT_POLICY)
    ...
    app = await createMcpServer({
      devToken: 'never-used-catalog-e2e',
      devPrincipal: {
        kind: 'user',
    ...
    const r = await fetch(`${baseUrl}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'catalog-e2e', version: '0' },
        },
      }),
    });
  ...
  // ---------------------------------------------------------------------------
  // 1. tools/list / catalog includes all 6 new catalog/tag tool names
  // ---------------------------------------------------------------------------
  it('tools/list includes all 6 new catalog/tag tool names', () => {
  ...
  //    list_tlds is allow-mode via list_* wildcard in DEFAULT_POLICY
  // ---------------------------------------------------------------------------
  it('operator calling list_tlds (allow-mode read, no args) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'list_tlds', {})) as {
    ...
    expect(
      ...
      'list_tlds should fail with openprovider_not_connected, got: ' + JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  // ---------------------------------------------------------------------------
  // 3. Allow read with body reaches handler → not-connected
  //    get_domain_price is allow-mode via get_* wildcard
  // ---------------------------------------------------------------------------
  it('operator calling get_domain_price (allow-mode read, with args) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'get_domain_price', {
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'get_domain_price should fail with openprovider_not_connected, got: ' + JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  // ---------------------------------------------------------------------------
  // 4. Allow write reaches handler → not-connected (proves allow, no confirm)
  //    create_tag is allow-mode (explicit entry in DEFAULT_POLICY)
  // ---------------------------------------------------------------------------
  it('operator calling create_tag (allow-mode write) reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'create_tag', {
      key: 'k',
      value: 'v',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'create_tag should fail with openprovider_not_connected (no confirm), got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);
  ...
  //    delete_tag is confirm-mode (explicit entry in DEFAULT_POLICY)
  // ---------------------------------------------------------------------------
  it('operator calling delete_tag (confirm-mode) → confirmation proposed, not executed', async () => {
    const sid = await initSession(operatorKey);
    const body = (await callTool(sid, operatorKey, 'delete_tag', {
      key: 'k',
      value: 'v',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      text,
      'delete_tag should return a confirmation proposal (result.content[0].text), got: ' +
        JSON.stringify(body),
    ).toBeDefined();
  ...
  // ---------------------------------------------------------------------------
  // 6a. Viewer write-gate: viewer calling delete_tag (confirm-mode, non-read)
  //     → policy_denied
  // ---------------------------------------------------------------------------
  it('viewer calling delete_tag (confirm-mode write) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'delete_tag', {
      key: 'k',
      value: 'v',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error,
      'viewer calling delete_tag should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // ---------------------------------------------------------------------------
  // 6b. Viewer write-gate: viewer calling create_tag (allow-mode write)
  //     → policy_denied
  // ---------------------------------------------------------------------------
  it('viewer calling create_tag (allow-mode write) → policy_denied', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'create_tag', {
      key: 'k',
      value: 'v',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error,
      'viewer calling create_tag should be denied, got: ' + JSON.stringify(body),
    ).toBeDefined();
    expect(body.error?.data?.code).toBe('policy_denied');
  }, 60_000);

  // ---------------------------------------------------------------------------
  // 6c. Viewer read-gate: viewer calling list_tlds (allow-mode read)
  //     → allowed, reaches handler → not-connected (same as operator path)
  // ---------------------------------------------------------------------------
  it('viewer calling list_tlds (allow-mode read) → allowed, reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'list_tlds', {})) as {
      result?: { content: { text: string }[] };
      error?: { message: string; data?: { code?: string } };
    };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'viewer calling list_tlds should reach handler and fail with openprovider_not_connected, got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);

  // ---------------------------------------------------------------------------
  // 6d. Viewer read-gate: viewer calling get_domain_price (allow-mode read)
  //     → allowed, reaches handler → not-connected
  // ---------------------------------------------------------------------------
  it('viewer calling get_domain_price (allow-mode read) → allowed, reaches handler → openprovider_not_connected', async () => {
    const sid = await initSession(viewerKey);
    const body = (await callTool(sid, viewerKey, 'get_domain_price', {
      domain: { name: 'x', extension: 'com' },
      operation: 'create',
    })) as {
      result?: { content: { text: string }[] };
    ...
    expect(
      body.error?.data?.code === 'openprovider_not_connected' ||
        (body.error?.message ?? '').toLowerCase().includes('not connected') ||
        JSON.stringify(body).toLowerCase().includes('not_connected'),
      'viewer calling get_domain_price should reach handler and fail with openprovider_not_connected, got: ' +
        JSON.stringify(body),
    ).toBe(true);
  }, 60_000);
});
